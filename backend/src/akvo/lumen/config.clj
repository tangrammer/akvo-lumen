...
(defn assert-bindings []
  ...
  (assert (:lumen-js-engine-max-memory env) (error-msg "LUMEN_JS_ENGINE_MAX_MEMORY"))
  (assert (:lumen-js-engine-max-time env) (error-msg "LUMEN_JS_ENGINE_MAX_TIME"))
...
(defn bindings []
  {'db-uri (:lumen-db-url env)
   ...
   'js-engine-max-memory (Integer/parseInt (:lumen-js-engine-max-memory env (str (* 2 1000000 1024))))
   'js-engine-max-time (Integer/parseInt (:lumen-js-engine-max-time env "2000"))})
